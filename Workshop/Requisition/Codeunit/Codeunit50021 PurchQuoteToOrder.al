codeunit 50021 "PurchQuoteToOrder(Yes/No)"
{
        ...
        if not IsHandled then
            if ConfirmManagement.GetResponseOrDefault(StrSubstNo(OpenNewOrderQst, PurchOrderHeader."No."), true) then
                ...
                if PurchOrderHeader."Purchase Type" = PurchOrderHeader."Purchase Type"::Local then
                    PAGE.Run(PAGE::"Local Purchase Order", PurchOrderHeader)
                else
                    PAGE.Run(PAGE::"Foreign Purchase Order", PurchOrderHeader)
    end;

    var
        ConvertQuoteToOrderQst: Label 'Do you want to convert the quote to an order?';
        PurchOrderHeader: Record "Purchase Header";
        PurchQuoteToOrder: Codeunit "Purch.-Quote to Order RBS";
