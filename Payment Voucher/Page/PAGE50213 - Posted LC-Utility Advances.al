page 50213 "Posted LC-Utility Adv.Requests"
{
    ApplicationArea = All;
    Caption = 'Posted LC Utility Advances';
    CardPageID = "PostedLC-Utility Adv.Req. Card";
    DeleteAllowed = false;
    Editable = false;
    InsertAllowed = false;
    PageType = List;
    ...
    SourceTableView = WHERE(Status = FILTER(<> Cancelled),
                            Posted = CONST(true), "Type of Advance" = filter(LC));
    UsageCategory = Lists;
...
Modify;
    ...
    begin
        ...
        rec.TestField("Account Type", Rec."Account Type"::Employee);
        ...
        if JTemplate = '' then begin
            Error('Ensure the LC Utility Advance Template is set up in Cash Office Setup');
        end;

        if JBatch = '' then begin
            Error('Ensure the LC Utility Advance Batch is set up in the Cash Office Setup')
        end;
