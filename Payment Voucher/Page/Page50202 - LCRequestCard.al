page 50202 "LC Request Card"
{
    Caption = 'LC Request Card';
    DeleteAllowed = false;
    PageType = Document;
    ApplicationArea = All;
    ...
    SourceTableView = WHERE("Payment Type" = CONST(LC), "PV Created" = filter(false));
    ...
    layout
    {
        area(content)
        {
            group(Control1)
            {
                ...
                {
                    ApplicationArea = All;
                    OptionCaption = ',,Cheque,EFT,Letter of Credit,,,';
            ...
            group(Functions)
            {
                ...
                action("Send A&pproval Request")
                {
                    ...
                    begin
                        ...
                        if ApprovalMgt.CheckPHeaderApprovalsWorkflowEnable(Rec) then
                            ApprovalMgt.OnSendPHeaderForApproval(Rec);
                    end;
                }
                action("Cancel Approval Re&quest")
                {
                    ...
                    var
                        ApprovalMgt: Codeunit "Approval Mgmt. ExtCal";
                    begin
                        ApprovalMgt.OnCancelPHeaderForApproval(Rec);
                    end;
                }
                ...
                action("Create PV")
                {
                    ...
                    begin
                        ...
                        PaymentsHeader."External Document No." := Rec."External Doc No.";
                        PaymentsHeader.Modify;

                        RecLines.Reset;
                        RecLines.SetRange("No.", Rec."No.");
                        if RecLines.Find('-') then
                            repeat
                                PaymentsLine.Init;
                                ...
                                PaymentsLine."Line No." += 10000;
                                PaymentsLine.Insert(true);
                                PaymentsLine."Account Type" := PaymentsLine."Account Type"::"Bank Account";
                                PaymentsLine.Validate("Account Type");
                                //PaymentsLine.Account := RecLines."Account Type"::"Bank Account";
                                PaymentsLine."Account" := PaymentsLine."Account"::Bank;
                                PaymentsLine.Validate(Account);
                                PaymentsLine.Validate("Account No.", RecLines."Account No.");
                                //PaymentsLine.Validate(PaymentsLine."Account Type", RecLines"Account Type");
                                ...
                                PaymentsLine."External Document No." := Rec."External Doc No.";
                                PaymentsLine.Modify(true);
                ...
                action(Post)
                {
                    ...
                    begin
                        ...
                        //end;
                        PaymentsLine.Reset;
                        PaymentsLine.SetRange("No.", Rec."No.");
                        if PaymentsLine.Find('-') then begin
                            repeat
                                LineNo := LineNo + 1000;
                                GenJnlLine.Init;
                                ...
                                GenJnlLine."Account No." := PaymentsLine."Account No.";
                                GenJnlLine.Validate(GenJnlLine."Account No.");
                                GenJnlLine.Description := 'Payment Request to' + PaymentsLine."Account Name" + '(' + PaymentsLine."Account No." + ')';
                                ...
                                GenJnlLine."Payment Request No." := Rec."No.";
