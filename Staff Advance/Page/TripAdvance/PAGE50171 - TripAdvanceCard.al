page 50171 "Trip Advance Card"
{
    Caption = 'Trip Advance Card';
    DeleteAllowed = false;
    PageType = Document;
    PromotedActionCategories = 'New,Approval,Approvals';
    ShowFilter = false;
    SourceTable = "Staff Advance Header";
    SourceTableView = WHERE(Posted = CONST(false), "Type of Advance" = filter("Trip Advance"));
    ...
    layout
    {
        area(content)
        {
            group("General Information")
            {
                ...
                field("No."; Rec."No.")
                {
                    ApplicationArea = all;    //Editable = CreateVouch;
                }
                field(Date; Rec.Date)
                {
                ...
                {
                    Caption = 'Trip Advance Account No.';
                    ApplicationArea = all;
                    trigger OnValidate()
                    var
                ...
                {
                    Caption = 'Trip Advance Account Name';
                    Editable = false;
                    ApplicationArea = all;
                }
            ...
            part("Trip Advance Lines"; "Trip Advance Subform")
            {
                Editable = editno;
                ApplicationArea = all;
            ...
            group(Approval)
            {
                ...
                action("Create Payment Voucher")
                {
                    ...
                    begin
                        ...
                        if not Confirm('Are you sure you want to create a Payment Voucher for %1', false, Rec."No.") then
                            ...
                            if StaffAdvLines.Findset then begin

                                EntryNo := 1;
                                repeat
                                    PaymentLines.Init;
                                    PaymentLines."Voucher Type" := PVHeadEr."Voucher Type";
                                    PaymentLines."Line No." := PaymentLines."Line No." + 10000;//EntryNo+1;
                                                                                               //MESSAGE('%1',EntryNo);
                                    PaymentLines."Document No." := PVHeadEr."No.";
                                    PaymentLines.Validate("Account Type", 6);

                                    PaymentLines.Validate("Account No.", Rec."Account No.");
                                    ...
                                    if BankRec.get(Rec."Account No.") then
                                        PaymentLines."Posting Group" := BankRec."Bank Acc. Posting Group";
                ...
                action(PostAdvance)
                {
                    ...
                    begin
                        ...
                        GenJnlLine.Description := 'Trip Advance: ' + Rec."Account No." + ':' + Rec.Payee;
