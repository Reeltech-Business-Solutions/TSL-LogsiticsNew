page 50066 "Staff Advance Request List"
{
    ApplicationArea = All;
    ...
    //SourceTableView = WHERE(Status = FILTER(<> Approved), Posted = CONST(false), "Account Type" = filter(Employee), "Type of Advance" = filter("Staff Advance"));
    UsageCategory = Lists;
        ...
        area(processing)
        {
            group("&Functions")
            {
                ...
                action(ImportStaffAdv)
                {
                    ...
                    trigger OnAction()
                    begin
                        Xmlport.Run(50004, true, true);
                        //  Message('Successful');
                    end;
                }
                action(ImportStaffAd)
                {
                    ApplicationArea = All;
                    Caption = 'Import Staff Advance Lines';
                    Image = Import;
                    Promoted = true;
                    PromotedCategory = Report;
                    PromotedIsBig = true;
    ...
    procedure PostAdvanceSelection()
    var
        Counter: Integer;
        Collection: Record "Staff Advance Header";
        CollectionList: Page "Staff Advance Request List"; //Dennis
    begin
