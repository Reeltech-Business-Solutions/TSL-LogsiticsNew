page 50095 "Posted Staff Advance Req. Card"
{

    Caption = 'Posted Staff Advance Req. Card';
    DeleteAllowed = false;
    PageType = Document;
    ShowFilter = false;
    Editable = false;
    ...
    SourceTableView = WHERE(Posted = CONST(true), "Type of Advance" = filter("Staff Advance"));
        ...
        area(processing)
        {
            group(Approval)
            {
                ...
                action(PrintPreview)
                {
                    ...
                    trigger OnAction()
                    begin
                        Rec.TestField(Status, Status::Posted);
