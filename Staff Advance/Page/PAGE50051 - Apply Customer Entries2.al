page 50051 "Apply Customer Entries2"
{
    ...
    begin
        ...
        // begin Added for receipt
        if CurrentRec then begin
            if CalcType = CalcType::Receipt then begin
                CurrPage.SetSelectionFilter(CustLedgEntry);
                CustEntrySetApplID.SetApplId(CustLedgEntry, ApplyingCustLedgEntry, GetAppliesToID)
                //ELSE //end Added for receipt
                // CustEntrySetApplID.SetApplId(CustLedgEntry,ApplyingCustLedgEntry,ReceiptLine."Applies-to ID");
            end;
        end;
                    ...
                    /*
                                            ApplnType::"Applies-to ID":
                                                begin
                                                    ...
                                                    HandleChosenEntries(2, ReceiptLine.Amount, ReceiptLine."Currency Code", ReceiptLine.Date);
    ...
    local procedure HandleChosenEntries(Type: Option Direct,GenJnlLine,SalesHeader; CurrentAmount: Decimal; CurrencyCode: Code[20]; PostingDate: Date)
    var
    ...
    begin
        ...
        if not AppliedCustLedgEntry.Find('-') then
            exit;
    ...
    [Scope('Cloud')]
    procedure ExchangeAmountsOnLedgerEntry(Type: Option Direct,GenJnlLine,SalesHeader; CurrencyCode: Code[20]; var CalcCustLedgEntry: Record "Cust. Ledger Entry"; PostingDate: Date)
    var
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterExchangeAmountsOnLedgerEntry(var CalcCustLedgEntry: Record "Cust. Ledger Entry"; CustLedgerEntry: Record "Cust. Ledger Entry"; CurrencyCode: Code[20])
    begin
    end;

    [IntegrationEvent(TRUE, false)]
    local procedure OnBeforeHandledChosenEntries(Type: Option Direct,GenJnlLine,SalesHeader; CurrentAmount: Decimal; CurrencyCode: Code[20]; PostingDate: Date; var AppliedCustLedgerEntry: Record "Cust. Ledger Entry"; var IsHandled: Boolean)
    begin
    end;
    ...
    procedure SetReceipts(NewReceiptLine: Record "Receipts Line"; var NewCustLedgEntry: Record "Cust. Ledger Entry"; ApplnTypeSelect: Integer)
    var
        TotalAdjCostLCY: Decimal;
    begin
