page 50178 "Trip Advance Ret. List"
{
    ApplicationArea = All;
    Caption = 'Trip Advance Retirement List';
    PageType = List;
    ...
    Editable = false;
    SourceTableView = WHERE(Posted = CONST(false), "Retirement Type" = filter("Trip Retirement"));
    //InsertAllowed = true;
        ...
        area(processing)
        {
            group(Functions)
            {
                ...
                action("Send Approval Request")
                {
                    ...
                    begin
                        //Ensure actual spent does not exceed the amount on original document
                        Rec.CalcFields(Rec."Actual Spent", "Cash Receipt Amount", Difference);
    ...
    begin
        ...
        //GenJnlLine."Bal. Account Type":=GenJnlLine."Bal. Account Type"::Employee;
        //GenJnlLine."Bal. Account No.":=ImprestDetails."Advance Holder";
        GenJnlLine.Description := 'Trip Advance Retirement';
        //GenJnlLine.VALIDATE(GenJnlLine."Bal. Account No.");
        GenJnlLine."Currency Code" := Rec."Currency Code";
        GenJnlLine.Validate("Currency Code");
        //Take care of Currency Factor
