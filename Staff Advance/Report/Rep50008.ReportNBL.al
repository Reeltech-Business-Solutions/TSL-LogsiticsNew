report 50014 "Sales NBL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './SalesDocumentNBL.rdl';
    Caption = 'Sales OVH';
            ...
            dataitem(PageCounter; "Integer")
            {
                ...
                dataitem(CopyLoop; "Integer")
                {
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(Sales_Line___TruckNo; "Sales Line"."Truck No.")
                        {
                        }
                        column(Sales_Line___TruckType; "Sales Line"."Truck Type")
                        {
                        }
                        column(Sales_Line___VaribleAmount; "Sales Line"."Varible Amount")
                        {
                        }
                        column(Sales_Line___FixedAmount; "Sales Line"."Fixed Amount")
                        {
                        }
                        column(Sales_Line___TotalDaysAvailable; "Sales Line"."Total Days Available")
                        {
                        }
                        column(Sales_Line___QuantityLoaded; "Sales Line"."Quantity Loaded")
                        {
                        }
                        column(Sales_Line___QuantityShortage; "Sales Line"."Quantity Shortage")
                        {
                        }
                        column(Sales_Line___ShortageTolerance; "Sales Line"."Shortage Tolerance")
                        {
                        }
                        column(Sales_Line___TotalDistanceCover; "Sales Line"."Total Distance Cover")
                        {
                        }
                        column(Sales_Line___TotalShortageAmount; "Sales Line"."Total Shortage Amount")
                        {
                        }
                        ...
                        begin
                            ...
                            if not "Sales Header"."Prices Including VAT" and
                               ("Sales Line"."VAT Calculation Type" = "VAT Calculation Type"::"Full VAT")
                            then
                            ...
                            if "Sales Line".Quantity <> 0 then begin
                                if "Sales Line"."No." = '' then
                                    AddError(StrSubstNo(Text019, "Sales Line".Type, "Sales Line".FieldCaption("Sales Line"."No.")));
                            ...
                            if not IsHandled then
                                if "Sales Line"."Line No." > OrigMaxLineNo then begin
                                    ...
                                    // TableID[1] := DimMgt.TypeToTableID3("Sales Line".Type.AsInteger());
                                    No[1] := "Sales Line"."No.";
                ...
                if ContractLine.FindFirst() then begin
                    repeat
                        ...
                        if ContractLine."Truck Code" <> '' then begin
                            ...
                            BillingLineSum.SetFilter("Transaction Date", '%1..%2', "Start Date", "End Date");
                ...
                repeat
                    ...
                    if SaleShptLine."Gen. Bus. Posting Group" <> SalesLine2."Gen. Bus. Posting Group" then
                        AddError(
                          StrSubstNo(
                            Text024,
                            SalesLine2.FieldCaption(SalesLine2."Gen. Bus. Posting Group")));
                    if SaleShptLine."Gen. Prod. Posting Group" <> SalesLine2."Gen. Prod. Posting Group" then
                        AddError(
                          StrSubstNo(
                            Text024,
                            SalesLine2.FieldCaption(SalesLine2."Gen. Prod. Posting Group")));
                    ...
                    if SaleShptLine."Job No." <> SalesLine2."Job No." then
                        AddError(
                          StrSubstNo(
                            Text024,
                            SalesLine2.FieldCaption(SalesLine2."Job No.")));
    ...
    begin
        ...
        // TableID[1] := DimMgt.SalesLineTypeToTableID(SalesLine.Type.AsInteger()); tolu
        No[1] := SalesLine."No.";
        TableID[2] := DATABASE::Job;
    ...
    begin
        ...
        case SalesLine2.Type of
                    ...
                    if SalesLine2."No." <> '' then
                        if Item.Get(SalesLine2."No.") then begin
                            ...
                            if Item.Reserve = Item.Reserve::Always then begin
                                SalesLine2.CalcFields(SalesLine2."Reserved Quantity");
                                if SalesLine2."Document Type" in [SalesLine2."Document Type"::"Return Order", SalesLine2."Document Type"::"Credit Memo"] then begin
                                    if (SalesLine2.SignedXX(SalesLine2.Quantity) < 0) and (Abs(SalesLine2."Reserved Quantity") < Abs(SalesLine2."Return Qty. to Receive")) then
                                        AddError(
                                          StrSubstNo(
                                            Text015,
                                            SalesLine2.FieldCaption(SalesLine2."Reserved Quantity"), SalesLine2.SignedXX(SalesLine2."Return Qty. to Receive")));
                                end else
                                    if (SalesLine2.SignedXX(SalesLine2.Quantity) < 0) and (Abs(SalesLine2."Reserved Quantity") < Abs(SalesLine2."Qty. to Ship")) then
                                        AddError(
                                          StrSubstNo(
                                            Text015,
                                            SalesLine2.FieldCaption(SalesLine2."Reserved Quantity"), SalesLine2.SignedXX(SalesLine2."Qty. to Ship")));
                            end
                        end else
                            AddError(
                              StrSubstNo(
            ...
            SalesLine2.Type::"Fixed Asset":
                    ...
                    if SalesLine2."No." <> '' then
                        if FA.Get(SalesLine2."No.") then begin
                            ...
                            if SalesLine2."Depreciation Book Code" = '' then
                                AddError(StrSubstNo(Text006, SalesLine2.FieldCaption(SalesLine2."Depreciation Book Code")))
                            else
                ...
                OnCheckSalesLineCaseTypeElse(SalesLine2.Type.AsInteger(), SalesLine2."No.", ErrorText);
                if ErrorText <> '' then
                    AddError(ErrorText);
            end;
        end;
    ...
    begin
        ...
        if SalesHeader."Bill-to Customer No." = '' then
            AddError(StrSubstNo(Text006, SalesHeader.FieldCaption(SalesHeader."Bill-to Customer No.")))
        else begin
    ...
    begin
        ...
        if SalesHeader."Posting Date" = 0D then
            AddError(StrSubstNo(Text006, SalesHeader.FieldCaption(SalesHeader."Posting Date")))
        else
            if SalesHeader."Posting Date" <> NormalDate(SalesHeader."Posting Date") then
                AddError(StrSubstNo(Text009, SalesHeader.FieldCaption(SalesHeader."Posting Date")))
            else begin
