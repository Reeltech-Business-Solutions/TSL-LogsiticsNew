report 50029 "Truck Transaction FMNL IDLE"
{
    ApplicationArea = All;
    Caption = 'FMNL Processing IDLETSL';
    DefaultLayout = RDLC;
    RDLCLayout = './Process Transaction FMNLIDLETSL.rdl';
    UsageCategory = Lists;
    dataset
    {
        dataitem(ContractAgreement; "Contract Agreement")
        {
            ...
            dataitem("Processed Billing Line"; "Processed Billing Line")
            {
                ...
                { }
                column(TotalDelay; TotalDelay)
                { }
                column(CST_Idle; "CST Idle")
                { }
                column(FMN_Idle; "FMN Idle")
                { }
                ...
                begin
                    ...
                    if MillageRange.FindFirst() then begin
                        FixedRate := MillageRange.Rate;
                        TotalAmountTrip := MillageRange."Fixed Rate" * ("CST Idle" + "FMN Idle");
                        VariableRate := MillageRange.Rate;
                    end;

                    TotalDelay := "CST Idle" + "FMN Idle";
